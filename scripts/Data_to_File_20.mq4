...
extern datetime Beginning = D'2013.03.01';
   ...
   for(i = size - 1; i >= 0; i--)
    {
      ...
      if(FromHour >= 0)
       {
         if(iTime(SymbolName, PeriodMinutes, i) >= Beginning && iTime(SymbolName, PeriodMinutes, i) <= TimeCurrent())
          {
            if(TimeHour(iTime(SymbolName, PeriodMinutes, i)) == FromHour && doubOpen == 0) 
             {
               strDate   = StringSubstr(TimeToStr(iTime(SymbolName, PeriodMinutes, i)), 0, 10);
               doubOpen  = iOpen(SymbolName, PeriodMinutes, i);
             } 
            ...
            if(TimeHour(iTime(SymbolName, PeriodMinutes, i)) >= FromHour && TimeHour(iTime(SymbolName, PeriodMinutes, i)) <= ToHour)   
             {
               if(iHigh(SymbolName, PeriodMinutes, i) > doubHigh) doubHigh = iHigh(SymbolName, PeriodMinutes, i);
               if(iLow(SymbolName, PeriodMinutes, i) < doubLow) doubLow = iLow(SymbolName, PeriodMinutes, i);
               doubVolume = doubVolume + iVolume(SymbolName, PeriodMinutes, i);
             }
            if(doubOpen != 0 && doubClose != 0) 
             {
               FileWrite(handle, strDate, doubOpen, doubHigh, doubLow, doubClose, doubVolume);
               strDate = ""; doubOpen = 0; doubHigh = 0; doubLow = 9999; doubClose = 0; doubVolume = 0;
             }  
